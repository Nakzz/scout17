...
sed -e "s/_scout17_/$db_name/g" cleandb_raw.sql | sed -e "s/_app_/$user_name/g" > cleandb.sql

sed -e "s/_scout17_/$db_name/g" createdb_raw.sql | sed -e "s/_password_/$passwd/g" | sed -e "s/_app_/$user_name/g" > createdb.sql
#TODO: the password appears in variable names and in comments and gets changed
# there too.  Fix it
sed -e "s/_scout17_/$db_name/g" infodb_raw.php | sed -e "s/_password_/$passwd/g" | sed -e "s/_app_/$user_name/g" > infodb.php
sed -e "s/_scout17_/$db_name/g" insert_event_raw.sql > insert_event.sql
